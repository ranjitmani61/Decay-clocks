...
def sample_node(db_session: Session) -> Node:
    node = Node(node_class=NodeClass.ML_MODEL, version_ref="v1", owner_team="risk",
                criticality=Criticality.HIGH, registration_time=datetime(2025,6,1,tzinfo=timezone.utc))
    db_session.add(node)
    db_session.commit()
    return node
...
class TestTemporalEscalation:
    async def test_dispatch_and_approve_escalation(self, db_session, sample_node, temporal_client):
        worker = Worker(temporal_client, task_queue=TASK_QUEUE, workflows=[HumanReviewWorkflow])
        worker_task = asyncio.create_task(worker.run())
        try:
            now = datetime(2026,1,1,12,0,0,tzinfo=timezone.utc)
            task_id = create_escalation_task(node_id=sample_node.node_id, team="risk",
                                             reason="regulatory floor breach", db=db_session,
                                             now=now, deadline_hours=24)
