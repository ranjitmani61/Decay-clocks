"""Updated tests for reliability dynamics (temporal axis fixed)."""
import math
import pytest
...
class TestTemporalFreshness:
    def test_no_elapsed(self):
        assert update_axis_temporal(0.8, 0, 0.01) == 1.0   # fresh when no time passed
    def test_half_life(self):
        alpha = 0.00693
        val = update_axis_temporal(0.0, 100, alpha)   # current_r_t ignored
        assert val == pytest.approx(0.5, abs=1e-3)
    def test_long_time_zero(self):
        val = update_axis_temporal(1.0, 10000, 0.01)
        assert val == 0.0
...
class TestComputeReliabilityVector:
    ...
    def test_multiple_axes(self):
        ...
        expected_t = math.exp(-0.01 * 5)   # pure time decay, ignoring current_r_t
        assert new_R[4] == pytest.approx(expected_t, abs=1e-3)
